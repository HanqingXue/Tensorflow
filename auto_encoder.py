...
def xavier_init(fan_in, fan_out, constant = 1):
	low  = -constant * np.sqrt(6.0 / (fan_out + fan_in))
	high = constant * np.sqrt(6.0 / (fan_out + fan_in))
	return tf.random_uniform((fan_in, fan_out), 
							minval = low, maxval = high,
							dtype = tf.float32)

class AdditiveGussianNoiseAutoencoder(object):
	"""docstring for AdditiveGussianNoiseAutoencoder"""
	def __init__(self, n_input, n_hidden, transfer_function=tf.nn.softplus,
				optimizer = tf.train.AdamOptimizer(), scale=0.1):
		super (AdditiveGussianNoiseAutoencoder, self).__init__()
...
def get_block_from_data(data, batch_size):
...
if __name__ == "__main__":
	...
	autoencoder = AdditiveGussianNoiseAutoencoder(n_input = 784,
					n_hidden = 400,
	...
	for epoch in range(training_epochs):
		...
		for i in range(total_batch):
			batch_xs = get_block_from_data(X_train ,batch_size)
	...
	feature_train =  autoencoder.transform(X_train)
	feature_test = autoencoder.transform(X_test)
	...
	predictions = classifier.predict(np.array(test))
	print predictions
	report = classification_report(np.array(test_label), predictions)
	print report

	classifier.fit(mnist.train.images, mnist.train.labels)
	predictions = classifier.predict(mnist.test.images)
	report = classification_report(mnist.test.labels, predictions)
	print report